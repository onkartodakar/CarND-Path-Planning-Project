        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			if(prev_size < 2){
				...
				ref_x = previous_path_x[9];
				ref_y = previous_path_y[9];
				
				double ref_x_prev = previous_path_x[8];
				double ref_y_prev = previous_path_y[8];
			...
			//In Frenet add evenly spaced points ahead of the reference
			...
			// push previous points as is
		    int prevPointsConsidered = 0;
		      
		    if( previous_path_x.size() > 10)
				prevPointsConsidered = 10;
		    else
				prevPointsConsidered = previous_path_x.size();

			//Start with all of the previous path points from the last time
			for(int i=0;i<prevPointsConsidered;i++){
				next_x_vals.push_back(previous_path_x[i]);
				next_y_vals.push_back(previous_path_y[i]);
			}

			//Calculate how to break up spline points so that we can travel at desired reference vel
			...
			//fill up rest of the points to make 50 points
			for(int i=1; i <= 50-prevPointsConsidered; i++){
			...
			/*cout<<"\n\n\n PRINTING WAYPOINTS PASSED:";
			for(int i=0; i<50; i++){
				cout<<"\n Waypoint -"<<i<<" X:"<<next_x_vals[i]<<" Y:"<<next_y_vals[i];
			}
			*/
			...
			for(int i = 0; i < 50; i++)
			{
				//Code for follow lane based on 'car_d'
				...
				//Starter code : Go in straight line
